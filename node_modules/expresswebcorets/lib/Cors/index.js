...
class Cors {
    ...
    getOrigin(origin) {
        ...
        if (this.options.origin instanceof RegExp &&
            this.isOriginAllowed(origin, [this.options.origin])) {
            return origin;
        }
        /**
        ...
        if (typeof this.options.origin === "string") {
            const isAllowed = this.isOriginAllowed(origin, this.options.origin.split(","));
        ...
        if (Array.isArray(this.options.origin)) {
            const isAllowed = this.isOriginAllowed(origin, this.options.origin);
            return isAllowed ? origin : false;
        }
        /**
     ...
     */
    setOrigin(origin, response) {
        if (!origin) {
            return this;
        }
        const allowedOrigin = this.getOrigin(origin);
    ...
    setHeaders(headers, response) {
        const corsHeaders = this.getHeaders(headers);
        if (corsHeaders) {
    ...
    async handle(request, response, next) {
        this.setOrigin(request.header("Origin"), response)
            .setCredentials(response)
            .setExposeHeaders(response);
        /**
        ...
        this.setMethods(response)
            .setHeaders(request.header("access-control-request-headers"), response)
            .setMaxAge(response);
