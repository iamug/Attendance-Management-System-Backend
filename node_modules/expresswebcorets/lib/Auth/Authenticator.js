...
class Authenticator extends AuthManager_1.default {
    ...
    async processRegistration(record) {
        return process.env.DB_CONNECTION == "mongoose"
            ? await this.registerNoSql(record)
            : await this.registerSql(record);
    }
    /**
    ...
    async innerLoginProcess(record) {
        return process.env.DB_CONNECTION == "mongoose"
            ? await this.loginNoSql(record)
            : await this.loginSql(record);
    }
    /**
     * Process user login
     * @param {Array} record
